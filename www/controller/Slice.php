...
class Slice
{
	...
	/*public function FormSliceData($extAlgArray, $extSliceCode,
	 $extApTableName, $extBpTableName, $extFlightId, $extParamNames)
	{
	$algArray = $extAlgArray;
	$apTableName = $extApTableName;
	$bpTableName = $extBpTableName;
	$flightId = $extFlightId;
	$sliceCode = $extSliceCode;
	$paramNames = $extParamNames;

	if(count($paramNames) != count($algArray))
	{
			...
			do
	{
			...
			$fieldCounter++;
			$queryData[$fieldCounter] = $row['value'];
