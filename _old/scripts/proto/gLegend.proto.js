...
//╋╋╋╋╋╋╋┗━━┛
//=============================================================
var CHART_PARAM_INFO_RECEIVE_LEGENT = 'rcvLegend';
...
function Legend(flightId, legendContainer, apParams, bpParams, associativeParamsArr,
	...
	this.scriptAddrDummyInfo = location.protocol + '//' + 
		location.host +	"/asyncParamInfo.php?action=" + CHART_PARAM_INFO_RECEIVE_LEGENT + 
		"&flightId=" + flightId + 
		"&paramCode=";
	this.paramInfo = new Array();
	this.dfr = $.Deferred();
	
	this.visirTimeBox = $('<div></div>').css({
...
Legend.prototype.ReceiveLegend = function(){
	...
	if(this.legendTitlesNotReceived) {
		var paramNames = self.apArr.concat(self.bpArr);
			scriptAddrInfo = this.scriptAddrDummyInfo + 
			paramNames.join('-');
...
Legend.prototype.CreateBarMainContainer = function(time, content, startId, endId, lineId) {
	 var self = this,
	 barMainLabel = $('<div/>', {
		id: 'barMainLabel' + (this.barMainContainersArr.length + 1),
		class: 'BarMainLabel',
		'data-time': time,
...
Legend.prototype.CreateLineContainer = function(time) {
	self = this;
	return $('<svg></svg>', {
		id: 'svgLines' + (this.linesContainersArr.length + 1),
		'data-time': time })
	.css({
		"top": '43px',
		"width": '1px',
...
Legend.prototype.CreateHorizont = function(yAxNum) {
	var e = this.CreateHorizontContainer(yAxNum),
		h = this.CreateHorizontValueContainer(yAxNum);
...
Legend.prototype.CreateHorizontContainer = function(yAxNum) {
	...
	return $('<svg></svg>', {
		...
		.css({
			"top": self.pos.pageY,
			"left": '14px',
			"width": self.ph.width() + this.ph.offset().left - 28,//left is negative num. 28 just pissing pix num
			"height": '1px',
			"position": 'absolute',
