	...
	($_SESSION['loggedIn'] === true))
{
	...
	{	
		...
		//not use because of posibility to be twice one file upload
		/*if($action == FLIGHT_GET_CUR_ID)
		{
			//update flight info
			if(isset($_POST['bort']) && ($_POST['bort'] != NULL))
			{
				$flightInfo['bort'] = $_POST['bort'];
			}
			
			if(isset($_POST['voyage']) && ($_POST['voyage'] != NULL))
			{
				$flightInfo['voyage'] = $_POST['voyage'];
			}
			...
			if(isset($_POST['bruType']) && ($_POST['bruType'] != NULL))
			{
				$flightInfo['bruType'] = $_POST['bruType'];
			}
				
			if(isset($_POST['performer']) && ($_POST['performer'] != NULL))
			{
				$flightInfo['performer'] = $_POST['performer'];
			}
				
			$Fl = new Flight();
			$flightInfo = $Fl->GetFlightExtendedInfo($flightInfo);
			unset($Fl);
			
			$flightId = $flightInfo["id"];
				
			echo(json_encode($flightId));
		}*/
