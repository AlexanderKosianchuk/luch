...
class FlightControls extends Component {
    handleClickTrash() {
        if (confirm(I18n.t('flights.flightControls.confirm'))) {
    ...
    buildButtons() {
        let control = (
            <span
                className={ 'flights-flight-controls__glyphicon glyphicon glyphicon-flag' }
        ...
        buttons.forEach(((item, index) => {
            if ((this.props.settings.flightShowAction === item.key)
                ...
                control = (
                    <span
                        className={ 'flights-flight-controls__glyphicon glyphicon ' + item.classModifyer }
    ...
    render() {
        return (
            <div className='flights-flight-controls'>
                { this.buildButtons() }
                <span
                    className={
                        'flights-flight-controls__glyphicon '
                        + 'flights-flight-controls__glyphicon-danger '
                        + 'glyphicon '
                        + 'glyphicon-trash'
                    }
